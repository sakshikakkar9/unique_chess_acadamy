...
const TournamentStudents: React.FC = () => {
  ...
  const filteredRegistrations = registrations.filter((reg) =>
    (reg.studentName || reg.student?.fullName || "").toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const handleExport = () => {
    ...
    const rows = filteredRegistrations.map(r => [
      ...
      r.category || "N/A",
      r.fideId || r.student?.fideId || "NA",
      r.fideRating || r.student?.fideRating || 0,
      r.phone || r.student?.phone,
      r.status
    ]);
  ...
  if (isTournamentLoading) {
    return (
      <AdminShell title="Loading..." subtitle="Fetching tournament roster">
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          <Button
            variant="ghost"
            size="sm"
            onClick={() => navigate(`/admin/tournaments/${id}/portal`)}
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-uca-bg-elevated/50 border-b border-uca-border">
                  <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-uca-text-muted">Ref ID</th>
                  <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-uca-text-muted">Player</th>
                  <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-uca-text-muted">Intel</th>
                      ...
                      <tr key={reg.id} className="hover:bg-uca-bg-elevated/30 transition-colors">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                              ...
                              <span className="text-[10px] text-uca-text-muted">
                                {reg.gender || reg.student?.gender} • {reg.dob ? format(new Date(reg.dob), "dd MMM yyyy") : 'N/A'}
                              </span>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex flex-col gap-0.5">
                            <span className="text-xs font-semibold text-uca-text-primary uppercase tracking-tight">{reg.category || 'Open'}</span>
                            <span className="text-[10px] text-uca-text-muted">FIDE: {reg.fideRating || reg.student?.fideRating || 0}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex flex-col gap-0.5">
                            <div className="flex items-center gap-1.5 text-xs text-uca-text-primary font-medium">
                              <Phone className="size-3 text-uca-accent-blue" /> {reg.phone || reg.student?.phone}
                            </div>
                            <div className="flex items-center gap-1.5 text-[10px] text-uca-text-muted">
                              <Mail className="size-3" /> {reg.email || reg.student?.email || 'N/A'}
                            </div>
                          </div>
                        </td>
