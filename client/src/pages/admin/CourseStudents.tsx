        ...
        <div className="flex items-center gap-4">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
               <Badge variant="outline" className="bg-blue-50 text-blue-600 border-blue-100 uppercase tracking-widest text-[9px] font-black">Course Roster</Badge>
      ...
      <div className="rounded-2xl border bg-white shadow-sm overflow-hidden">
        <Table>
          <TableHeader className="bg-slate-50">
            <TableRow>
              <TableHead className="font-black text-[10px] uppercase tracking-widest text-slate-400 py-4">ID</TableHead>
              <TableHead className="font-black text-[10px] uppercase tracking-widest text-slate-400">Student Name</TableHead>
              <TableHead className="font-black text-[10px] uppercase tracking-widest text-slate-400">Category & Level</TableHead>
            ...
            {isRegLoading ? (
              ...
              filteredEnrollments.map((reg) => (
                <TableRow key={reg.id} className="hover:bg-slate-50/50 transition-colors">
                  <TableCell className="font-mono text-[10px] font-bold text-blue-600">{reg.id.substring(0, 8)}...</TableCell>
                  <TableCell>
                    <div className="font-bold text-slate-900">{reg.studentName}</div>
                    <div className="text-[10px] text-slate-400 font-medium">{reg.gender} • {reg.dob ? format(new Date(reg.dob), "dd MMM yyyy") : 'N/A'}</div>
                  </TableCell>
                  <TableCell>
                    <div className="space-y-1">
                       <Badge variant="secondary" className="text-[9px] font-black uppercase bg-slate-100 text-slate-600 border-none">{reg.category || 'General'}</Badge>
                       <div className="text-[10px] font-bold text-slate-500">Level: {reg.experienceLevel}</div>
                    </div>
                  </TableCell>
                  <TableCell>
